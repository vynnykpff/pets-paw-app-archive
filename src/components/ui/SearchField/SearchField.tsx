...
const SearchField = () => {
  ...
  const handleFiltered = () => {
    if (inputRef.current) {
      ...
      if (inputValue) {
        const data = breedsData.filter(item => item.breedName.toLowerCase().includes(inputValue.toLowerCase()));
  ...
  return (
    ...
    >
      <input ref={inputRef} placeholder="Search for breeds by name" className={styles.searchInput} type="text" onKeyDown={handleKeyPress} />
