...
const asyncThunk = createAsyncThunk("breeds/get_current_breed_images", async function (arg: BreedsCurrentImage, { rejectWithValue }) {
	try {
		return await BreedsService.getCurrentBreedImage(arg.breed, arg.limit);
	} catch (error: any) {
...
const storeHandler: StoreAsyncThunkHandler<BreedsState> = (state, action) => {
	state.breedsData = action.payload.map((image: IBreedsItem) => ({ url: image.url, id: image.id, breedName: image.breeds[0].name }));
